...
def check_is_file(file_path):
    ...
    if not os.path.isfile(file_path):
        return generate_response(warning='File not found. Please check the file path passed in.')
    return generate_response(result=True)

def approximate_line_count(file_path, encoding='utf-8'):
...
def sample_file(file_path, sample_size=100, skip_first_nlines=1,encoding='utf-8', random_seed=42):
    ...
    if is_file['result']:
        ...
        if isinstance(total_num_lines_res['result'], int): # successfully estimated line count of the file
            ...
            with open(file_path, 'rb') as f:
                ...
                if total_num_lines / sample_size < 1.5: 
...
if __name__ == "__main__":
    print(sample_file(r'C:\Users\zyx199199\Desktop\a.txt',1,1))